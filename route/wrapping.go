...
`)
	...
	err = g.generateCallOnErr(name, nil, wrap.PkgPath, wrap.Type, wrap.Requests, wrap.Responses, errName, true)
	if err != nil {
		return err
	}
	g.buf.WriteString(`
...
func (g *GenRoute) generateResponseErrorReturn(errName string, code string, noFmtErr bool) error {
	if !noFmtErr {
		for _, wrap := range g.api.Wrappings {
			...
			err = g.generateResponseError(errName, "500", true)
			if err != nil {
				return err
			}
			err = g.generateResponseBodyItem(resp.Name, resp.Type, code, resp.Content, errName, true)
			if err != nil {
				return err
			}
			return nil
