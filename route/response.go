...
func (g *GenRoute) generateResponsesVar(name string, resps []*spec.Response) error {
	...
	for _, resp := range resps {
		...
		g.buf.WriteFormat(`// responses %s %s.%s
var %s `, resp.Type.PkgPath, name, resp.Name, g.getVarName(resp.Name, resp.Type))
		g.Types(resp.Type)
		g.buf.WriteString("\n")
	}
