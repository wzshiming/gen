...
func (g *GenModel) convertToBool(in, out string, typ *spec.Type) error {
	g.buf.AddImport("", "strconv")
	g.buf.WriteFormat(`
	if _%s, err := strconv.ParseBool(%s); err == nil {
		%s = `, out, in, out)
	g.Types(typ)
...
func (g *GenModel) convertToFloat64(in, out string, typ *spec.Type) error {
	g.buf.AddImport("", "strconv")
	g.buf.WriteFormat(`
	if _%s, err := strconv.ParseFloat(%s, 0); err == nil {
		%s = `, out, in, out)
	g.Types(typ)
	g.buf.WriteFormat(`(_%s)
	}
...
`, out0, out)
	...
	switch typ.Kind {
	case spec.Bool:
		return g.convertToBool(in, out, typ)
	case spec.String:
	...
	case spec.Float32, spec.Float64:
		return g.convertToFloat64(in, out, typ)
	case spec.Slice:
